"""
Профиль: обновление данных, загрузка аватара, выдача верификации (admin), список пользователей
"""
import json
import os
import base64
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        # GET / or /users — список всех пользователей (онлайн-статус)
        if method == 'GET' and (path.endswith('/users') or path == '/'):
            cur.execute("""
                SELECT id, username, display_name, is_verified, is_admin, avatar_url, last_seen
            ...
            for r in rows:
                ...
                users.append({
                    ...
                    'is_verified': r[3], 'is_admin': r[4], 'avatar_url': r[5],
        ...
        if method == 'PUT' and path.endswith('/update'):
            ...
            return {
                ...
                'body': json.dumps({'user': {'id': r[0], 'username': r[1], 'display_name': r[2], 'is_verified': r[3], 'is_admin': r[4], 'avatar_url': r[5]}})
            }
        ...
        # POST /verify — выдать верификацию (только admin)
        ...
        # POST /make-admin — сделать пользователя администратором (только admin)
        if method == 'POST' and path.endswith('/make-admin'):
            if not user or not user['is_admin']:
                return {'statusCode': 403, 'headers': CORS_HEADERS, 'body': json.dumps({'error': 'Нет прав'})}
